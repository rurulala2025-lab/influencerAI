...
const ResultGallery: React.FC<ResultGalleryProps> = ({ stories }) => {
  ...
  return (
    <div className="mt-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-8 pl-2 border-l-4 border-purple-500 flex items-center gap-3">
        Generated Stories
        <span className="text-sm font-normal text-zinc-600 dark:text-zinc-500 bg-zinc-100 dark:bg-zinc-900 px-2 py-1 rounded-md border border-zinc-200 dark:border-zinc-800">
        ...
        {stories.map((story) => {
          ...
          return (
            <div key={story.id} className="relative">
              {/* Story Header */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white dark:bg-zinc-900/50 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-2 text-xs font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg transition-colors flex items-center gap-2"
                    >
                      {isAllSelected ? <CheckCircle2 size={14} className="text-purple-500 dark:text-purple-400"/> : <Circle size={14} />}
                    ...
                    <button
                        ...
                        className="px-3 py-2 text-xs font-medium text-zinc-600 dark:text-zinc-300 border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors flex items-center gap-2"
                      >
                        <Download size={14} />
                        Download All
                    </button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {story.images.map((img, index) => {
                    ...
                    return (
                      <div 
                        key={img.id} 
                        className={`group relative rounded-xl overflow-hidden shadow-lg transition-all duration-300 border-2 ${isSelected ? 'border-purple-500 scale-[0.98]' : 'border-zinc-200 dark:border-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600'}`}
              ...
              {/* Divider */}
              <div className="mt-16 border-b border-zinc-200 dark:border-zinc-900"></div>
            </div>
          );
        })}
      </div>
